...
CONF_OK=$(echo "$INTENT_CONFIDENCE >= 0.6" | bc -l 2>/dev/null || echo "0")
[[ "$CONF_OK" != "1" ]] && exit 0
...
if [[ -f "$PROMPT_FILE" ]]; then
    ...
    if echo "$LAST_PROMPT" | grep -qE "(do it|go ahead|make the change|implement|fix it|update it|change it|apply|execute|run it|send it|ship it)"; then
